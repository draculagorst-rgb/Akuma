...
const primstart = async () => {
    ...
    prim.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            if (config().status.reactsw && mek.key && mek.key.remoteJid === 'status@broadcast') {
                ...
                prim.sendMessage('status@broadcast', { react: { text: sigma, key: mek.key } }, { statusJidList: [mek.key.participant] });
            }
    ...
    prim.deleteMessage = async (chatId, key) => {
        try { await prim.sendMessage(chatId, { delete: key }); } 
        catch (error) { console.error('Failed to delete message:', error); }
    };

    prim.sendText = async (jid, text, quoted = '', options) => {
        prim.sendMessage(jid, { text: text, ...options }, { quoted });
    };
    ...
    prim.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        await prim.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted });
        return buffer;
    };
    ...
    prim.albumMessage = async (jid, array, quoted) => {
        const album = generateWAMessageFromContent(jid, { albumMessage: { expectedImageCount: array.filter(a => a.hasOwnProperty("image")).length, expectedVideoCount: array.filter(a => a.hasOwnProperty("video")).length } }, { userJid: prim.user.jid, quoted, upload: prim.waUploadToServer });
        await prim.relayMessage(jid, album.message, { messageId: album.key.id });
        for (let content of array) {
            const img = await generateWAMessage(jid, content, { upload: prim.waUploadToServer });
            img.message.messageContextInfo = { messageSecret: crypto.randomBytes(32) };
            await prim.relayMessage(jid, img.message, { messageId: img.key.id, quoted: { key: { remoteJid: album.key.remoteJid, id: album.key.id, fromMe: true, participant: prim.user.jid }, message: album.message } });
        }
        return album;
    };
    ...
    prim.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        ...
        let m;
        try { m = await prim.sendMessage(jid, message, { ...opt, ...options }); } catch (e) { console.error(e); m = null; } finally { if (!m) m = await prim.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options }); return m; }
    };

    prim.sendStatusMention = async (content, jids = []) => {
        let users;
        for (let id of jids) { let userId = await prim.groupMetadata(id); users = userId.participants.map(u => prim.decodeJid(u.id)); }
        let message = await prim.sendMessage("status@broadcast", content, { backgroundColor: "#000000", font: Math.floor(Math.random() * 9), statusJidList: users });
        jids.forEach(id => { prim.relayMessage(id, { groupStatusMentionMessage: { message: { protocolMessage: { key: message.key, type: 25 } } } }, {}); delay(2500); });
        return message;
    };
...
fs.watchFile(file, () => { delete require.cache[file]; require(file); });
...
console.error = function (msg, ...args) { if (typeof msg === 'string' && ignoredErrors.some(e => msg.includes(e))) return; originalConsoleError.apply(console, [msg, ...args]); };
